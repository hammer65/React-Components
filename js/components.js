...
/*
*  FmtDate: US date time
*  Set value with string Date or set timestamp with unix timestamp
*  Set just format for current date;
*/

var FmtDate = React.createClass({
  format: function(){
    var stamp = this.props.timestamp ? this.props.timestamp : Date.now();
    var fmt = this.props.format ? this.props.format : "m-d-y"
    if(this.props.value){
      stamp = Date.parse(this.props.value);
    }
    return TagUtility.date(fmt,stamp);
  },
  render: function(){
    return React.DOM.span({className:"fmt-date"},this.format());
  }
});
...
var TagUtility = {
  /*
  * PHP style date format from php.js
  */
